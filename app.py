...
def proxy_m3u():
    ...
    # Estrai header personalizzati dalla query string
    custom_headers_from_query = {
        unquote(key[2:]).replace("_", "-"): unquote(value).strip()
        for key, value in request.args.items()
        if key.lower().startswith("h_")
    }
    
    # Unisci gli header di default con quelli personalizzati dalla query
    # Gli header dalla query sovrascrivono quelli di default se ci sono conflitti
    current_request_headers = {**default_headers, **custom_headers_from_query}

    try:
        # Risolvi l'URL (potrebbe essere un link diretto o richiedere la logica iframe)
        resolved_data = resolve_m3u8_link(original_m3u_url, headers=current_request_headers)
        m3u_url_to_fetch = resolved_data.get("resolved_url")
        # Usa gli header restituiti da resolve_m3u8_link per la richiesta successiva
        # Questi potrebbero essere stati modificati (es. Referer, Origin) durante la risoluzione
        headers_for_fetching_m3u = resolved_data.get("headers", current_request_headers) 

        if not m3u_url_to_fetch:
            return f"Errore: Impossibile risolvere l'URL M3U: {original_m3u_url}", 500

        print(f"Fetching M3U from resolved URL: {m3u_url_to_fetch} with headers: {headers_for_fetching_m3u}")
        response = requests.get(m3u_url_to_fetch, headers=headers_for_fetching_m3u, allow_redirects=True, timeout=(10,30))
        response.raise_for_status()
        final_url = response.url # URL dopo eventuali redirect del GET finale
        m3u_content = response.text

        file_type = detect_m3u_type(m3u_content)

        if file_type == "m3u": # Lista IPTV generica
            # Per le liste M3U semplici, non modifichiamo i link interni, solo il content type
            return Response(m3u_content, content_type="audio/x-mpegurl")

        # Se è un M3U8 (flusso HLS)
        parsed_url = urlparse(final_url)
        base_url = f"{parsed_url.scheme}://{parsed_url.netloc}{parsed_url.path.rsplit('/', 1)[0]}/"

        # Prepara la query string degli header da passare ai segmenti TS e alle chiavi
        # Usa gli header usati per scaricare l'M3U8 finale (headers_for_fetching_m3u)
        headers_query_for_segments = "&".join([f"h_{quote(k)}={quote(v)}" for k, v in headers_for_fetching_m3u.items()])

        modified_m3u8 = []
        for line in m3u_content.splitlines():
            line = line.strip()
            if line.startswith("#EXT-X-KEY") and 'URI="' in line:
                line = replace_key_uri(line, headers_query_for_segments) # Passa gli header corretti
            elif line and not line.startswith("#"):
                # Se il link è relativo, costruisci l'URL assoluto
                segment_url = urljoin(base_url, line)
                # Proxy per il segmento TS, passando gli header
                line = f"/proxy/ts?url={quote(segment_url)}&{headers_query_for_segments}"
        ...
        print(f"Errore RequestException in /proxy/m3u: {str(e)}")
        traceback.print_exc()
        return f"Errore durante il download del file M3U/M3U8: {str(e)}", 500
    except Exception as e:
        print(f"Errore generico in /proxy/m3u: {str(e)}")
        traceback.print_exc()
        return f"Errore generico durante l'elaborazione del M3U/M3U8: {str(e)}", 500
